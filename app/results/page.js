...
export default function ResultsPage() {
  ...
  const parsed = useMemo(() => {
    try {
      ...
      // Coerce to numbers 1–5; ignore invalids
      const vals = arr
        .map((v) => Number(v))
  ...
  const metrics = useMemo(() => {
    ...
    const mean = sum / n;
    const pct = ((mean - 1) / 4) * 100; // map 1–5 to 0–100
      ...
      <div className="mx-auto max-w-4xl px-6 py-10">
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <p className="text-xs text-slate-500">میانگین (۱ تا ۵)</p>
            <p className="mt-2 text-3xl font-semibold text-slate-900">
              {metrics.mean.toFixed(2)}
            </p>
          </div>
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <p className="text-xs text-slate-500">امتیاز نرمال‌شده (۰ تا ۱۰۰)</p>
            <p className="mt-2 text-3xl font-semibold text-slate-900">
              {metrics.pct.toFixed(0)}%
            </p>
          </div>
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <p className="text-xs text-slate-500">جمع کل مقادیر</p>
            <p className="mt-2 text-3xl font-semibold text-slate-900">{metrics.sum}</p>
          </div>
        </div>
